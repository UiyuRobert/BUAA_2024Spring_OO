...
public class Adjust {
    public static String simplifyString(String input) {
        String inputInfo = input.replaceAll("\\s","");
    ...
    private static BigInteger parseCoefficient(String coefficient) {
        // 如果系数为空，表示为1
    ...
    private static String simplifyTerm(BigInteger coefficient, int exponent) {
        // 处理系数和指数，构建化简后的单项式
    ...
    public static String simplifyPoly(String input) {
        Pattern pattern = Pattern.compile("([+-]?\\d*)\\*x\\^(\\d+)\\*exp");
        ...
        while (matcher.find()) {
            ...
            if (result.length() > 0 && !simplifiedTerm.isEmpty()) {
