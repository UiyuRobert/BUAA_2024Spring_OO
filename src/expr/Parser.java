...
public class Parser {
    ...
    public Expr parseExpr() throws IOException, ClassNotFoundException {
        Expr expr = new Expr();
        int sign = 1;
    ...
    public Term parseTerm(int sign) throws IOException, ClassNotFoundException {
    ...
    public Factor parseFactor() throws IOException, ClassNotFoundException {
        if (lexer.peek().getType() == Token.Type.DX) {
            lexer.nextToken(); // 跳过 dx
            lexer.nextToken(); // 跳过 (
            Factor insideDerived = parseExpr().derive();
            lexer.nextToken(); // 跳过 )
            return insideDerived;
        }
    ...
    public Factor parseFuncFactor(String funcName) throws IOException, ClassNotFoundException {
        ArrayList<Factor> actualParas = new ArrayList<>(); // 实参集合
        Factor factor = parseFactor();
        if (factor == null) {
            actualParas.add(new Num("0"));
        } else {
            actualParas.add(factor);
        }
